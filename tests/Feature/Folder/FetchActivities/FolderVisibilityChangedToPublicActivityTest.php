...
class FolderVisibilityChangedToPublicActivityTest extends TestCase
{
    ...
    {
        $this->createFolderActivity = new CreateFolderActivity(ActivityType::FOLDER_VISIBILITY_CHANGED_TO_PUBLIC);
    }
    ...
    public function fetch(): void
    {
        ...
        $this->fetchActivitiesTestResponse(['folder_id' => $folder->public_id->present()])
            ...
            ->assertJsonPath('data.0.type', 'FolderVisibilityChangedToPublicActivity')
            ->assertJsonPath('data.0.attributes.message', 'Jack Sparrow changed folder visibility to public')
    ...
    public function whenVisibilityWasChangedByAuthUser(): void
    {
        ...
        $this->fetchActivitiesTestResponse(['folder_id' => $folder->public_id->present()])
            ->assertOk()
            ->assertJsonCount(1, 'data')
            ->assertJsonPath('data.0.attributes.message', 'You changed folder visibility to public');
    }
}
